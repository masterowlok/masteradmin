...
const SubscriptionList = (props) => {
  ...
  return ([item.title,item.amount,item.currency,item.offer_name,item.description,moment(item.createdAt).utc().format('llll'),moment(item.updatedAt).utc().format('llll')]);
    ...
    const getUserList = async () => {
        try {
          setisLoading(true)
            const res=await axios.get(`${REACT_APP_API_PORT}api/admin/subscription/list`,
            {
              headers: {
    ...
    //    }
        ...
        .then((result) => {
          if (result.isConfirmed) {
            Swal.fire(
              ...
              return axios.get(`${REACT_APP_API_PORT}api/admin/subscription/remove/${id}`,{
              headers: {
                      'Content-Type': 'application/json',
                      'x-access-token': admin.jwtoken
                    },
      ...
      const column = [
        ...
        {
          name:<h1 className='text-lg'>Title</h1>,
     ...
     
        {
          ...
          selector: (row) => (
            <div className="flex items-center space-x-3.5">
              <button className="hover:text-primary"  onClick={()=>navigate(`/subscription/view/${row._id}`)}
              title='view'
              >
                <svg
              ...
              <button className="hover:text-primary"  
                   title='Edit'
          onClick={()=>navigate(`/subscription/edit/${row._id}`)}
              >
 ...
 <DefaultLayout>
   ...
   <div className='hidde s:block px-5 pt-3 flex gap-3 overflow-aut flex-wrap'>
       <form action='https://formbold.com/s/unique_form_id' method='POST'>
        ...
        <CSVLink data={tableArray} headers={headers} filename={'Subscription Details.csv'}>
