...
return ([ item.main_category_id?.
  ...
  item.status,moment(item.createdAt).utc().format('llll'),moment(item.updatedAt).utc().format('llll')]);
})
 ...
 console.log(status)
  ...
  row.status === "Active"?true:false
 ...
 
    {
      ...
      selector: (row) => (
        ...
        {
          (subcategoryModule?.update === 'Active' || auth === 'admin') &&
  ...
  <Breadcrumb pageName="All SubCategory List" />
     ...
     <button
           ...
           className='inline-flex mt-1 items-center justify-center rounded-md bg-primary py-2 px-6 text-center font-medium text-white hover:bg-opacity-90 lg:px-6 xl:px-6 text-sm'
         >
           CSV
         </button>
         </CSVLink>
