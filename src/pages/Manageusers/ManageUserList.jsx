...
const ManageUserList = (props) => {
  ...
  return ([item.name,item.email,item.phone,item.status,item.address,moment(item.createdAt).utc().format('llll'),moment(item.updatedAt).utc().format('llll')]);
})
      ...
      row.status === "Active"?true:false
     ...
     
        {
          ...
          selector: (row) => (
            ...
            {
              (manageUserModule?.update === 'Active' || auth === 'admin') &&
          ...
          onClick={()=>navigate(`/user/edit/${row._id}`)}
            ...
            {
              (manageUserModule?.delete === 'Active' || auth === 'admin') &&
  ...
  return (
   ...
   <div className='hidde s:block px-5 pt-3 flex gap-3 overflow-aut flex-wrap'>
       <form action='https://formbold.com/s/unique_form_id' method='POST'>
        ...
        <CSVLink data={tableArray} headers={headers} filename={'User Details.csv'}>
