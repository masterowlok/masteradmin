...
const  CouponList = (props) => {
  ...
  return ([item.title,item.amount,item.currency,item.offer_name,item.description,moment(item.createdAt).utc().format('llll'),moment(item.updatedAt).utc().format('llll')]);
})

    const getUserList = async () => {
        try {
          setisLoading(true)
            const res=await axios.get(`${REACT_APP_API_PORT}api/admin/coupon/list`,
            {
              headers: {
         ...
         .then((result) => {
            console.log(result)
             if (result.isConfirmed) {
                return  axios.get(`${REACT_APP_API_PORT}api/admin/coupon/statusUpdate/${id}`,
               {
                 headers: {
                   'Content-Type': 'application/json',
                   'x-access-token': admin.jwtoken
    ...
    //   delete user by admin
        ...
        .then((result) => {
          if (result.isConfirmed) {
            Swal.fire(
              ...
              return axios.get(`${REACT_APP_API_PORT}api/admin/coupon/remove/${id}`,{
              headers: {
                      'Content-Type': 'application/json',
                      'x-access-token': admin.jwtoken
                    },
      ...
      const column = [
        ...
        {
          name:<h1 className='text-lg'>Coupon Code</h1>,
          selector:(row)=>row.coupon_code,
          sortable:true,
          },
      
        {
          name:<h1 className='text-lg'>Amount</h1>,
     ...
     
        {
          ...
          selector: (row) => (
            <div className="flex items-center space-x-3.5">
              <button className="hover:text-primary"  onClick={()=>navigate(`/coupon/view/${row._id}`)}
              title='view'
              >
                <svg
              ...
              <button className="hover:text-primary"  
                   title='Edit'
          onClick={()=>navigate(`/coupon/edit/${row._id}`)}
              >
 ...
 <DefaultLayout>
     ...
     {isLoading?(<DataLoading/>):(<>
       <Breadcrumb pageName="Coupon List" />
   ...
   <div className='hidde s:block px-5 pt-3 flex gap-3 overflow-aut flex-wrap'>
       <form action='https://formbold.com/s/unique_form_id' method='POST'>
        ...
        <CSVLink data={tableArray} headers={headers} filename={'Coupon Details.csv'}>
