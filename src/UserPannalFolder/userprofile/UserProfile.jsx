...
const UserProfile = (props) => {
  ...
  const getData=async()=>{
    setisLoading(true)
  try {
    const res=await axios.get(`${REACT_APP_API_PORT}api/userauth/getprofile`,
  {  
    headers: {
      'Content-Type': 'application/json',
      'x-access-token': user?.jwtoken
    }}
    )
        ...
        <div className='px-4 pb-6 text-center lg:pb-8 xl:pb-11.5'>
          <div className='relative z-30 mx-auto -mt-22 h-30 w-full max-w-30 rounded-full bg-white/20 p1 backdrop-blur sm:h-44 sm:max-w-44 sm:p3'>
            <div className='relative drop-shadow-2 '>
              <img src={`${REACT_APP_API_PORT}uploads/${data?.image}`!==null &&
                             `${REACT_APP_API_PORT}uploads/${data?.image}`!=='null'?
                             `${REACT_APP_API_PORT}uploads/${data?.image}` : userSix} alt='profile' className='rounded-full h-30 w-30 sm:h-44 sm:w-[300px]' />
              {/* <label
                htmlFor='profile'
