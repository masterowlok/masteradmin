...
router.get('/list',verifyToken, async function(req, res, next){
  try{
        const data = await rolename.find().populate('userId',{name:1}).sort({"createdAt":-1}).exec();
...
router.get('/activerole',verifyToken, async function(req, res, next){
    try{
          const data = await rolename.find({status:'Active'}).populate('userId',{name:1}).sort({"createdAt":-1}).exec();
          return res.status(200).json({ success:'Data found', data:data });
    }catch(err){
      return res.status(500).json({ errors: err });
    }
...
router.post('/create',verifyToken,
  ...
  try{
    var getData = await rolename.findOne({'role_name':req.body.role_name}).exec();
      if (getData) {
      return res.status(400).json({ errors: "Role Already Exist" });
    }
    let updateData = { 
...
router.get('/statusUpdate/:id',verifyToken, async function(req, res, next){
        let dataId= req.params.id;
  try{
       const viewDatas= await rolename.findOne({'_id':dataId}).exec();
    if(viewDatas){
      ...
      if(statusKey == 'Active'){
       ...
       await rolename.findOneAndUpdate({'_id':dataId}, {'status':newStatusKey});
    }
